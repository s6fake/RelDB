...
public class SearchController implements Initializable {
    ...
    private final String personCommand = "SELECT n.id, n.name, TO_CHAR(i.info) AS birthday, CASE WHEN (n.gender = 'm') THEN 'male' ELSE CASE WHEN (n.gender = 'f') THEN 'female' END END AS gender FROM IMDB.name n JOIN IMDB.person_info i ON n.id = i.person_id";

    private final String characterCommand = "SELECT DISTINCT n.id AS ID, cn.name AS CHAR_NAME, n.name AS NAME, n.id AS TITLE, n.id AS YEAR FROM  IMDB.cast_info ci\n"
            + "JOIN IMDB.char_name cn ON ci.person_role_id = cn.id\n"
            + "JOIN IMDB.name n ON ci.person_id = n.id";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cb_characters.selectedProperty().set(false);
        cb_characters.setDisable(true);
        cb_companies.selectedProperty().set(false);
        cb_companies.setDisable(true);
    }
    ...
    private void on_simpleSearch(ActionEvent event) {
        if (!textField_keywords.getText().isEmpty()) {
            ...
            String characterCondition = convertInput("cn.name");
            ...
            Reldb_Statement statement3 = new Reldb_Statement(RELDB_02.getConnection());

            ResultSet titleResults = null, personResults = null, characterResult = null;
            ...
            if (cb_characters.selectedProperty().getValue()) {
                characterResult = statement3.executeQuery(characterCommand + " " + characterCondition, 25);
            }

            parent.newResultTab(textField_keywords.getText(), titleResults, personResults, characterResult);
            statement1.close();
            statement2.close();
            statement3.close();
